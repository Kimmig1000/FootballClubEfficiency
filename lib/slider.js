// source https://github.com/bobhaslett/d3-v4-sliders
    ...
    function drawSlider(p) {
        ...
        function init() {
            ...
            if (orient == "horizontal") {
                track.append("line")
                    .attr("class", "track")
                    .attr("x1", parseNum(sliderScale.range()[0]))
                    .attr("x2", parseNum(sliderScale.range()[1]))
                    .select(function () {
                        return this.parentNode.appendChild(this.cloneNode(true));
                    })
                    .attr("class", "track-overlay")
                    .attr("x1", parseNum(sliderScale.range()[0]))
                    .attr("x2", parseNum(sliderScale(value)))
            }
            if (orient == "vertical") {
                track.append("line")
                    .attr("class", "track")
        ...
        function reposition() {
            ...
            if (scale) {
                ...
                let axis = svg.selectAll(".sliderAxis")
                    .call(d3[axisType](sliderScale)
                        .tickSize(8).tickFormat(d3.format("d"))
                        .tickValues(calculateTicks(range, ticks))
                    );
            }
        ...
        function calculateTicks(range, interval) {
            let intervals = d3.range(parseNum(range[0]), parseNum(range[1]), interval);
            intervals.push(parseNum(range[1]))
            return intervals
        }
